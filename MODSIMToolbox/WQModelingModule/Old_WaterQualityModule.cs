...
namespace RTI.CWR.MODSIM.WQModelingModule
{
    ...
    public class MODSIMWQuality
    {
        ...
        public List<List<int>> nodeCalcOrder { get; private set; }
        ...
        public MODSIMWQuality(ref Model myModel, bool calibMode = false, bool ReservoirANNTrnsp = false, bool useMeasConc = false) //IApplication m_app = default
        {
        ...
        public void OnInitilize(string m_DataModelDB)
        {
            ...
            //    if (((QualityNodeData)cur_Node.Tag).InflowConcentration.getSize() > 0)
            //    {
            ...
            //        object a = ((QualityNodeData)cur_Node.Tag).InflowConcentration.getSize();
            //    }
            ...
            //m_OutputSupport.AddCurrentUserDemandOutput += addNodeQualityOutput;
            //m_OutputSupport.AddCurrentUserNonStorageOutput += addNodeQualityOutput;
            //m_OutputSupport.AddCurrentUserReservoirOutput += addReservoirQualityOutput; // addNodeQualityOutput
        ...
        private void CalcFTNConc(Node FTN)
        {
            ...
            if (FTN.mnInfo.infLink.mlInfo.flow>0)
            {
                if (((QualityNodeData)FTN.Tag).InflowConcentration.getSize() > 0 & FTN.mnInfo.inflow.Length > 0)
                {
                    sumConc = FTN.mnInfo.inflow[ts, 0] * ((QualityNodeData)FTN.Tag).Concentrations[ts, 0];
                    ((QualityNodeData)FTN.Tag).FTDAdjusted = true;
                }
                foreach (Node mNode in ((QualityNodeData)FTN.Tag).myFTDNodes)
                    sumConc += ((QualityNodeData)m_Node.Tag).INConcentrations(ts, 0) * mNode.mnInfo.ithruNF * mNode.m.idstrmfraction[0];
